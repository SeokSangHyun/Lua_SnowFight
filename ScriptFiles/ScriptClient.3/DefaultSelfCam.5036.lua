...
if not Script.IsCanMove then 
    --PC 환경에서 사용할 조작키 이벤트
    Input:AddAxisKeyEvent("SelfCamInput", "SelfCamTurn", Enum.Key.MouseX, 1)
    Input:AddAxisKeyEvent("SelfCamInput", "SelfCamLookUp", Enum.Key.MouseY, -1)
    Input:AddAxisKeyEvent("SelfCamInput", "SelfCamZoomInOut", Enum.Key.MouseWheelAxis, -50)
    Input:AddActionKeyEvent("SelfCamInput", "SelfCamShoot", Enum.Key.SpaceBar)
    
    --모바일 환경에서 사용할 조작키 이벤트
    Input:AddAxisKeyEvent("SelfCamInput", "SelfCamTurn_M", Enum.Key.Touch_ScreenX, 1)
    Input:AddAxisKeyEvent("SelfCamInput", "SelfCamLookUp_M", Enum.Key.Touch_ScreenY, 1)
    Input:AddAxisKeyEvent("SelfCamInput", "SelfCamZoomInOut_M", Enum.Key.Screen_Pinch, 1)
end

--PC 환경에서 사용할 조작키 이벤트
Input:AddActionKeyEvent("SelfCamInput", "SelfCamShoot", Enum.Key.LeftShift)
Input:AddActionKeyEvent("SelfCamInput", "SelfCamShoot", Enum.Key.Enter)
Input:AddActionKeyEvent("SelfCamInput", "SelfCamExit", Enum.Key.Escape)
...
if IsMobileTouchScreen == false then 
    LocalPlayer:ProcessInputAxisEvent("SelfCamTurn", function(value) 
        LocalPlayer:Turn(value)
    end)
    
    LocalPlayer:ProcessInputAxisEvent("SelfCamLookUp", function(value)
        LocalPlayer:LookUp(value)
    end)
    
    LocalPlayer:ProcessInputAxisEvent("SelfCamZoomInOut", function(value)
        LocalPlayer:ZoomInOut(value)
    end)
    
    LocalPlayer:ProcessInputActionEvent("SelfCamShoot", Enum.KeyInputType.Pressed, function()
        LocalPlayer:TakeSelfCamera()
    end)
        
    LocalPlayer:ProcessInputActionEvent("SelfCamExit", Enum.KeyInputType.Pressed, function()
        LocalPlayer:ExitSelfCamera()
    end) 
    
--모바일이면
else
    LocalPlayer:ProcessTouchAxisEvent("SelfCamTurn_M", function(value)
       local rotspeed = 4.0
       LocalPlayer:Turn(value * rotspeed)     
    end)
    
    LocalPlayer:ProcessTouchAxisEvent("SelfCamLookUp_M", function(value)
       local rotspeed = 4.0
       LocalPlayer:LookUp(value * rotspeed)       
    end)
    
    LocalPlayer:ProcessTouchAxisEvent("SelfCamZoomInOut_M", function(value)     
